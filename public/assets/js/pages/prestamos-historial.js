  ...
  tblPrestamos = $("#tblPrestamos").DataTable({
    ...
    columns: [
          ...
          if (type === "display") {
            return `<li class="list-group-item">${
              data.identidad + ": " + data.num_identidad
                        ...
                        <li class="list-group-item">Fecha: ${data.fecha[0]}${data.fecha[1]}${data.fecha[2]}${data.fecha[3]}${data.fecha[4]}
                        ${data.fecha[5]}${data.fecha[6]}${data.fecha[7]}${data.fecha[8]}${data.fecha[9]}</li>
      ...
      { data: null,
        render: function (data, type) {
          if (type === "display") {
            return `<li class="list-group-item">Nº VIN: ${
               data.vin
            }</li>
                        <li class="list-group-item"> Vehículo: ${
                          data.id_fabricante + " " + data.id_modelo
                        }</li>
                        
                        <li class="list-group-item">Precio de Venta: ${
                          Intl.NumberFormat({ style: 'currency'}).format(data.precio) + " MXN"
                        }</li>

                        <li class="list-group-item">Enganche: ${
                          Intl.NumberFormat({ style: 'currency'}).format(data.enganche) + " MXN"
                        }</li>`;
          }
          return data;
        },
      ...
      { data: "usuario" },
      {
        ...
        render: function (data, type) {
          if (type === "display") {
            if(data.estado == 1){
              return `<span class="badge bg-danger">Activo</span>`;
            }else{
