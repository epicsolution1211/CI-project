    ...
    public function detail($id)
    {
        ...
        $count_now = $this->detalle
            ...
            if($count_now==0){
                ...
                $this->detalle->selectSum('installment')->where(['estado' => 1])->first()['installment'];
                

                $data['outstanding_balance_with'] =
                $this->detalle->selectSum('installment')->where(['estado' => 1])->first()['installment']+
                $this->detalle->selectSum('interest')->where(['estado' => 1])->first()['interest'];
               
                    
                $data['outstanding_balance_without_pay'] =
                $this->detalle->selectSum('installment')->where(['estado' => 1])->first()['installment']+$detaildata['interest'];
        ...
        $data['next_pay'] = $detail? $detail['installment']:0;
        $data['next_pay_with'] =$detail ? $detail['installment']+$detail['interest'] : 0;
        ...
        if ($this->request->is('POST')) {
            ...
            if($count_now==0){
                ...
                return redirect()->to(base_url('prestamos/' . $id_prestamo . '/detail'))->with('respuesta', [
                   ...
                   if($custompayamount<$current_amount){
                    $row=$this->detalle->where([
                        'estado'=>1,
                        'id'=>$id
                    ])
                    ->set('paid_installment', ($custompayamount+$current_paid))
                    ->set('installment', 'installment -'.$custompayamount, false)
                    ->update();
                   }
                   else if($custompayamount==$current_amount){
                        $now = $this->detalle
                        ->where([
                            'estado'=> 1, 
                            'id'=>$id
                        ...
                        ->set([
                            'estado'=> 0,
                            'installmnent'=>0,
                            'paid_installment'=>$current_amount+$current_paid
                            ])
                        ->update();
                    ...
                    if($custompayamount>=$allsub_installment){
                        $now = $this->detalle
                            ->where(['id'=>$id])
                            ->set([
                                'estado'=> 0, 
                        ...
                        $paid_count=(int)((
                            $custompayamount
                            -$current_amount)/($index['importe_cuota']-$index['interest']));

                        $dif=$custompayamount-$current_amount-($index['importe_cuota']-$index['interest'])*$paid_count;
                        ...
                        ->set(
                            'installment', 'installment -' . $dif, false
                        )
                        ->update();
    ...
    public function listHistorial()
    {
        if ($this->request->is('get')) {
            ...
            for ($i = 0; $i < count($data); $i++) {
                ...
                $outstanding_valance =$this->detalle->selectSum('installment')->where(['estado' => 1])->first()['installment']+
                $this->detalle->selectSum('interest')->where(['estado' => 1])->first()['interest'];
                }
